...
### Subgroups
1. $|G|$ denotes the order of $G$ which is the number of elements in $G$.
2. $H \le G$ if and only if: 
	1. $H$ is closed under the binary operation of $G$
	2. the identity element $e$ of $G$ is in $H$
	3. $\forall a\in H$ it's true that $a^{-1} \in H$ also
3. *minimal* and *smallest*: A subset $H$ of $S$ is minimal with respect to the property if $H$ has the property, and no subset $K\subset H, K\ne H$ has the property.
4. For any an element $a$ in $G$. Any $H\le G$ must contains the group $\set{a^n\mid n\in \mathbb{Z}}$, which is the *smallest* subgroup of $G$. We call this smallest subgroup **cyclic subgroup of $G$ generated by $a$**, denoted by $\langle a\rangle$ .

### Cyclic Groups
1. We define  *the order of* the generator $a$ by the order of generated cyclic subgroup $|\langle a \rangle|$.
2. Every cyclic group is **abelian**.
3. **Division Algorithm for $\mathbb{Z}$**: $\forall m\in \mathbb{Z}^{+}, n\in \mathbb{Z}$ there must exist unique $q, r\in \mathbb{Z}$ such that: $$ n = mq + r\text{ and } 0\le r \lt m$$ $q$ : *quotient*, $r$ : *remainder*
4. The *gcd*(Greatest Common Divisor) of $r, s\in \mathbb{Z}$ is the generator of $H = \set{nr+ns\mid n,m\in\mathbb{Z}}$ 
5. Let $G=\langle a\rangle$, if $\langle G\rangle\lt\infty$ , then $G$ is isomorphic to $\langle \mathbb{Z},+\rangle$, or to $\langle \mathbb{Z}_n,+_n\rangle$.
6. $|\langle a\rangle|$ equals the smallest integer $m$ satisfying $a^m=e$.
7. $G = \langle a\rangle, |G| =n$, let $H=\langle a^m\rangle, H\le G$, then $|H| = n/d, d=gcd(n,m)$.
8. $G=\langle a\rangle$, then other generator of $G$ are $a^r$, where $r$ is relatively prime to $n$.